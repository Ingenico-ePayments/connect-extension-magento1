...
class Ingenico_Connect_Model_Order_EmailManager implements Ingenico_Connect_Model_Order_EmailInterface
{
    ...
    protected $statusMapping = array(
        'action_needed' => array(
            Ingenico_Connect_Model_Ingenico_StatusInterface::PENDING_PAYMENT
        ),
        'payment_successful' => array(
            Ingenico_Connect_Model_Ingenico_StatusInterface::CAPTURE_REQUESTED,
            Ingenico_Connect_Model_Ingenico_StatusInterface::CAPTURED,
            Ingenico_Connect_Model_Ingenico_StatusInterface::PAID
        ),
        'payment_rejected' => array(
            Ingenico_Connect_Model_Ingenico_StatusInterface::REJECTED,
            Ingenico_Connect_Model_Ingenico_StatusInterface::REJECTED_CAPTURE
        ),
        'fraud_suspicion' => array(
            Ingenico_Connect_Model_Ingenico_StatusInterface::PENDING_FRAUD_APPROVAL
        ),
        'delayed_settlement' => array(
            Ingenico_Connect_Model_Ingenico_StatusInterface::PENDING_APPROVAL
        ),
        'slow_3rd_party' => array(
            Ingenico_Connect_Model_Ingenico_StatusInterface::REDIRECTED
        )
    );

    /**
     * Ingenico_Connect_Model_Order_EmailManager constructor.
     */
    ...
    public function process(Mage_Sales_Model_Order $order, $ingenicoPaymentStatus)
    {
        ...
        && $ingenicoPaymentStatus != Ingenico_Connect_Model_Ingenico_StatusInterface::PENDING_PAYMENT
        ) {
            return;
        }

        $info = Mage::helper('ingenico_connect')->getPaymentStatusInfo($ingenicoPaymentStatus);
        $instructions = null;
        if ($ingenicoPaymentStatus == Ingenico_Connect_Model_Ingenico_StatusInterface::PENDING_PAYMENT) {
